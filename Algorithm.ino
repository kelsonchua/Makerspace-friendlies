    ...
    case SPEED_RUN: {
      ...
      if (position >= 0) {         // Following line
        ...
        if (pathIndex >= path.size()) {   // Finish path
          ...
          if (duration < 500) { // If unexpected error happen
            currentState = WAITING;
      ...
      if (digitalRead(button) == LOW) {  // If lost track
        unsigned long duration = buttonCheck();
...
int calculatePosition() { 
  ...
  sensorState = tempState; // Update global state for junction checks elsewhere

  // --- Decision Logic based on Sensor Pattern ---
...
char handleJunction() {
  unsigned long startTime = millis();
  bool canGoLeft = (sensorState & 0b11000); 
  bool canGoRight = (sensorState & 0b00011);   
  
  while (position == -2) {
    if (millis() - startTime >= 1000) { // Reach goal
      motor_drive(0, 0);
      currentState = WAITING;
      Serial.println("Goal!!");
      send("Goal!!");
  ...
  bool canGoForward = (sensorState & 0b00100); 
