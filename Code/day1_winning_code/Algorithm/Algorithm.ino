...
int error = 0;
int ExpSpeed = 120; // Exploration speed (increased for faster response)
int speed = 80;    // Speed run speed
const float motorBalance = 1.0; // Adjust if one motor is stronger (e.g., 0.95 for weaker right motor)

// Time
const int GOAL_TIME = 1000;
const int FREE_TIME = 200;
const int TIME_OUT = 1500;
...
void loop() {
  switch (currentState) {
    ...
    case START: {
      ...
      if (position == -2) {   // Found junction/dead end
