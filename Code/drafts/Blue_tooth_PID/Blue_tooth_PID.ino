...
void setup() {
  ...
  SerialBT.begin("ESP32_PID_Tuner"); // Name of your device
...
void loop() {
  // --- Bluetooth PID Tuner Logic ---
  if (SerialBT.available()) {
    String incoming = SerialBT.readStringUntil('\n');
    incoming.trim(); // Remove spaces/newlines
    
    if (incoming.length() > 1) {
      char type = toupper(incoming[0]); // P, I, or D
      float value = incoming.substring(1).toFloat(); // The number after the letter
      
      if (type == 'P') { Kp = value; SerialBT.printf("Updated Kp to: %.3f\n", Kp); }
      else if (type == 'I') { Ki = value; SerialBT.printf("Updated Ki to: %.3f\n", Ki); }
      else if (type == 'D') { Kd = value; SerialBT.printf("Updated Kd to: %.3f\n", Kd); }
      else if (type == 'S') { ExpSpeed = (int)value; SerialBT.printf("Updated Speed to: %d\n", ExpSpeed); }
    }
  }
...
int calculatePosition() { 
  ...
  for (int i = 0; i < 5; i++) {
    ...
    int weight = map(val, 0, 4095, 0, 1000); 
      ...
      switch (currentState) {
        ...
        case EXPLORATION_MAZE: {
          if (sensorState == 0b11111) {
            ...
            if (millis() - goalTime > 60) {         // Check timer
              motor_drive(0, 0);
              send("Goal Reached (in Phase 1)!!");
              currentState = EXPLORATION_FREE;
              send("Mode: Exploration_free");
              return 'N';
            }
          } else {
