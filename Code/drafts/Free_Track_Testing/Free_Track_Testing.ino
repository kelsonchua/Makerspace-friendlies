...
int error = 0;
int ExpSpeed = 75; // Exploration speed (increased for faster response)
...
enum RobotState {
  ...
  EXPLORATION_MAZE,
  EXPLORATION_FREE,        // Mapping the maze
  START_SPEED_RUN,    // Go through starting line
  SPEED_RUN           // Following the optimized path
};
RobotState currentState = EXPLORATION_FREE;    // Initial state
...
void loop() {
  ...
  if (start) {
    ...
    else if (position == -1) {   // Found junction/dead end
      Serial.println("Junction detected");
      send("Junction detected");
      char junction = free_junction();
...
void turn(char move, int speed) {
  if (move == 'L') {
    ...
    while (true) {
      calculatePosition();
      motor_drive(-0.9*speed, 0.9*speed);
...
char free_junction() {
  ...
  send("Start checking");
  while (position == -1 && sensorState != 0b00000) {
    motor_drive(ExpSpeed, ExpSpeed); // Keep moving forward
    position = calculatePosition();
